...
async def on_ready():
    ...
    for s in bot.guilds:
      print(f'{s} - {s.id}')
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.streaming, name="discord.gg/danone"))
...
@bot.command()
@commands.has_role(1086703139723427880)
async def ban(ctx, user,*, reason=None):
    ...
    try:
      embed = discord.Embed(title=f'**Da None Moderation**', description=f'BANNED By ({ctx.author}): \n\nUSER ID: `{plrusernamefunc}`\n\nBan Reason: **{reason}**\n\nUnban key: **__{this}__**', color=discord.Color.red())
      chan = await bot.fetch_channel(1087484092133675139)
...
async def unban(ctx, trelloident,*, reason=None):
  ...
  chan = await bot.fetch_channel(1087467955278848082)
